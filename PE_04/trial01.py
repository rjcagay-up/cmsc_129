...
def update_line_numbers():
    ...
    i = editor_frame.index("@0,0")
    while True:
        dline = editor_frame.dlineinfo(i)
        if dline is None:
            break
        y = dline[1]
        linenum = str(i).split(".")[0]
        line_numbers.create_text(2, y, anchor="nw", text=linenum, fill="blue")
        i = editor_frame.index("%s+1line" % i)

def on_editor_scroll(*args):
    update_line_numbers(editor_frame, line_numbers)
...
line_numbers = tk.Canvas(main_frame, width=30, bg="lightgrey")
line_numbers.grid(row=1, column=0, sticky="nsew")

# Editor section
editor_frame = ScrolledText(main_frame, wrap=tk.WORD, font=("Courier New", 12))
...
# Bind the editor_frame's scrolling event to update line numbers
editor_frame.bind("<Configure>", lambda event: update_line_numbers())
editor_frame.bind("<KeyPress>", lambda event: update_line_numbers())
editor_frame.bind("<MouseWheel>", lambda event: on_editor_scroll(*event.get()))
editor_frame.bind("<Button-4>", lambda event: on_editor_scroll("scroll", "-1", "units"))
editor_frame.bind("<Button-5>", lambda event: on_editor_scroll("scroll", "1", "units"))
